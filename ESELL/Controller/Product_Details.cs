...
namespace Controller
{
    public class Product_Details
    {
        ...
        public string getnameshop(object id)
        {
            try
            {
                int idcat = Utils.CIntDef(id);
                var list = db.ESHOP_SHOPs.Where(n => n.ID == idcat).ToList();
                if (list.Count > 0)
                    return list[0].NAME;
                return "";
            }
        ...
        public string getnameshopbyseourl(object s)
        {
            try
            {
                string seourl = Utils.CStrDef(s);
                var list = db.ESHOP_SHOPs.Where(n => n.SEO_URL == seourl).ToList();
                if (list.Count > 0)
                    return list[0].NAME;
                return "";
            }
        ...
        public string getShopSeourl(int idth)
        {
            var list = db.ESHOP_SHOPs.Where(n => n.ID == idth).Select(n => new { n.SEO_URL }).ToList();
            if (list.Count > 0)
            {
                return "/shop/" + list[0].SEO_URL + "/";
            }
            return "";
        }
        ...
        public void Addsee(string _news_seo)
        {
            ...
            if (list.Count > 0)
            {
                //if (String.IsNullOrEmpty(Utils.CStrDef(HttpContext.Current.Session["seeIP"])))
                //{
                    ...
                    //HttpContext.Current.Session["seeIP"] = HttpContext.Current.Request.UserHostAddress;
                //}
            }
        ...
        {
            var list = db.ESHOP_PROPERTies.Where(n => n.PROP_PARENT_ID == id&&n.PROP_RANK==3).ToList();
            return list;
        }
        ...
        public string getAreaDesc(int id)
        {
            string s = "";
            var list = db.ESHOP_PROPERTies.Where(n => n.PROP_ID == id).ToList();
            if (list.Count > 0)
        ...
        //Load title
        public string Loadtitle(string _news_seo_url)
        {
            try
            {
                var list = db.ESHOP_NEWs.Where(n => n.NEWS_SEO_URL == _news_seo_url).Select(n => new { n.NEWS_TITLE }).ToList();
                if (list.Count > 0)
                {

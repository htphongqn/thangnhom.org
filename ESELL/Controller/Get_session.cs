...
namespace Controller
{
    public class Get_session
    {
        ...
        public void LoadShopInfo(string News_Seo_Url)
        {
            try
            {

                var news = db.GetTable<ESHOP_SHOP>().Where(c => c.SEO_URL == News_Seo_Url);

                if (news.ToList().Count > 0)
                {

                    #region Bind News Info
                    HttpContext.Current.Session["Shop_id"] = news.ToList()[0].ID;
                    HttpContext.Current.Session["Shop_title"] = news.ToList()[0].NAME;
                    HttpContext.Current.Session["Shop_seo_keyword"] = news.ToList()[0].SEO_KEYWORD;
                    HttpContext.Current.Session["Shop_seo_desc"] = news.ToList()[0].SEO_DESC;
                    HttpContext.Current.Session["Shop_seo_url"] = news.ToList()[0].SEO_URL;
                    HttpContext.Current.Session["Shop_seo_title"] = news.ToList()[0].SEO_TITLE;
                    HttpContext.Current.Session["Shop_type"] = news.ToList()[0].TYPE;
                    HttpContext.Current.Session["Shop_field1"] = news.ToList()[0].FIELD1;
                    HttpContext.Current.Session["Shop_field2"] = news.ToList()[0].FIELD2;
                    HttpContext.Current.Session["Shop_field3"] = news.ToList()[0].FIELD3;
                    HttpContext.Current.Session["Shop_field4"] = news.ToList()[0].FIELD4;
                    HttpContext.Current.Session["Shop_field5"] = news.ToList()[0].FIELD5;
        ...
        public bool check_newsExits(string _url)
        {
            var list = db.ESHOP_NEWs.Where(n => n.NEWS_SEO_URL == _url).ToList();
            if (list.Count > 0)
                return true;
            return false;
        }
